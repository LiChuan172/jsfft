...
import { skewness as _skewness, kurtosis as _kurtosis } from "./formulae"
...
export class Signal extends ComplexArray {
  ...
  InvFFT() {
    if (this.domain === "time") {
      throw new Error("Already in the time domain")
    } else {
      this.domain = "time"
      return super.InvFFT()
    }
  ...
  // 峭度 kurtosis
  get kurtosis() {
    return _kurtosis(this.real)
  }

  // sample kurtosis
  get sampleKurtosis() {
    return _sampleKurtosis(this.real)
  }
